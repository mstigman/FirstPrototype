...
contract ReviewSystem {
    ...
    function writeReview (string memory content, uint reviewableID, uint reviewTokenID) public returns (bool){
        ...
        require(!reviewable.consumersReviewed[msg.sender]);
    ...
    function createReward(uint reviewableID, bytes memory data, uint amount) payable public {
        ...
        function getReviewIDsFromReviewable(uint reviewableID, uint amount) public view returns (uint[] memory reviewIDs) { //rank needs to be in contract!!! this nust return the top reviews update on vote
            Reviewable memory r = referenceReviewables[reviewableID];
            require(r.created);
            return r.reviewIDs;
        }
